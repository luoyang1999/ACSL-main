...
class LVISEval:
    ...
    def accumulate(self):
        ...
        num_thrs = len(self.params.iou_thrs) # array([0.5 , 0.55, 0.6 , 0.65, 0.7 , 0.75, 0.8 , 0.85, 0.9 , 0.95])
        num_recalls = len(self.params.rec_thrs) # [0:0.01:1]
        num_cats = len(cat_ids) # [1:1:1230]
        num_area_rngs = len(self.params.area_rng) # [[0, 10000000000.0], [0, 1024], [1024, 9216], [9216, 10000000000.0]]
        num_imgs = len(self.params.img_ids) # 5000
        ...
        self.eval = {
            "params": self.params,
            "counts": [num_thrs, num_recalls, num_cats, num_area_rngs],  # [10, 101, 1230, 4]
            "date": datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "precision": precision, # [10, 101, 1230, 4]
            "recall": recall, # [10, 1230, 4]
        ...
        #       value: dict
        #           key: ['dt_ids', 'tps', 'fps']
        #           value: int(n,), bool(10, n), bool(10, n)
    ...
    def print_results(self):
        ...
        for key, value in self.results.items():
            ...
            print(
                template.format(_type, iou, area_rng, max_dets, cat_group_name,
                                value * 100))
